...
class OmegaInitialObject n where
  initialize :: Omega Z n
instance OmegaInitialObject Z where
  initialize = IdZ
...
-- prodLimitInOmegaZZ = TerminalUniversal (PairNat IdZ IdZ) $ OmegaNat (\(PairNat f s) -> f) (\cpt (PairNat () s) -> undefined)
